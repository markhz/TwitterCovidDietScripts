...
# 
# # mclapply -- auto batch across cores (8)
# 
# tic() # 47 seconds for 1000 tweets
# pred_test_full <- tweets_in %>%
#   map_df(~ predictTweet(.x,
#                         fit_forest = fit_forest,
#                         tSparseRef = tSparseRef,
#                         train_terms = train_terms))
# toc()
# colnames(pred_test_full) <- "consumption"
# sum(pred_test_full$consumption==1) / nrow(pred_test_full)
# 
# 
# tic()
# write.csv(pred_test_full, "Data/ProcessedData/prediction_full.csv",
#           row.names = FALSE)
# toc()
# 
# 
...
# -------------------------------------------------------------------------
# predict all tweets once
# -------------------------------------------------------------------------


# tic()
# corpus_full <- generateCorpus(df_in$tweet)
# toc() # ~ 30 min
# 
# rm(df_in)
# 
# #Creating Document term matrix
# tic()
# frequencies_full = DocumentTermMatrix(corpus_full)
# toc() # ~ 8 min
# 
# rm(corpus_full)
# 
# tic()
# frequencies_full = as.data.frame(as.matrix(frequencies_full))
# # colnames(tSparse_full) = make.names(colnames(tSparse_full))
# toc() # 
# 
# 
# 
...
# # fit tweet DTM to training terms
# tic()
# excludeTerms_full <- setdiff( colnames(frequencies_full) , train_terms)
# tSparse_full <- bind_rows(tSparseRef, frequencies_full) %>% 
#   select(-excludeTerms_full) %>%
#   replace(is.na(.), 0)
# toc()
# 
# # predict food consumption tweet 
# tic()
# pred_test_full = predict(fit_forest, newdata = frequencies_full)
# toc()
# 
# tic()
# write.csv(pred_test_full, "Data/ProcessedData/prediction_full.csv",
